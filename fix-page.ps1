                                ...
                                <Card className="overflow-hidden mx-4 gap-y-0 ring-0">
                                    ...
                                    <CardContent>
                                        <div className="flex items-center gap-1 py-4 -mx-6 px-6 overflow-x-auto scrollbar-hide">
                                            <Button size="sm" className={`h-7 gap-1.5 text-xs shrink-0 ${selectedCategoryId === null ? "" : "text-muted-foreground hover:text-foreground"}`} onClick={() => handleCategoryClick(null)}>
                                                All <span className="opacity-70">{total}</span>
                                            </Button>
                                            {categories.map((category) => (
                                                <Button key={category.id} variant={selectedCategoryId === category.id ? "default" : "ghost"} size="sm" className={`h-7 gap-1.5 text-xs shrink-0 ${selectedCategoryId === category.id ? "" : "text-muted-foreground hover:text-foreground"}`} title={category.description} onClick={() => handleCategoryClick(category.id)}>
                                                    {category.name}
                                                </Button>
                                            ))}
                                        </div>
                                            ...
                                            articles.map((article) => (
                                                <Link key={article.id} href={`/article/${article.id}`} className="block group">
                                                    <div className="flex items-start gap-2">
                                                        {article.isTop && (
                                                            <Badge variant="outline" className="h-5 rounded-full border-red-500/30 text-[10px] text-red-600 dark:text-red-400 shrink-0">置顶</Badge>
                                                        )}
                                                        <span className="text-[10px] text-muted-foreground shrink-0">{article.categoryName || "未分类"}</span>
                                                        <span className="text-[10px] text-muted-foreground shrink-0">{formatDate(article.createTime)}</span>
                                                    </div>
                                            ...
                                            <Pagination>
                                                <PaginationContent>
                                                    ...
                                                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                                                        <PaginationItem key={page} className={page === currentPage ? "" : "hidden sm:inline-flex"}>
...
"@
[System.IO.File]::WriteAllText("c:\Users\Rog\Desktop\myblog-web-react\app\myblog\page.tsx", $content, [System.Text.Encoding]::UTF8)