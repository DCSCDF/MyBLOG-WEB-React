...
function CodeEditor({
    ...
    const prevWritingRef = React.useRef(writing)
    ...
    const codeToHighlight = !writing ? code : visibleCode
    const isDone = !writing || isAnimDone
    ...
    React.useEffect(() => {
        if (!(codeToHighlight.length && isInView)) {
            return
        }
        ...
        loadHighlightedCode()
    }, [lang, themes, isInView, codeToHighlight, resolvedTheme])
